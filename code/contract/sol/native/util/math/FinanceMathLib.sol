...
library FinanceMathLib {
    ...
    function bestNetAssetValueWithoutAssetBalance(Payload payload) internal view returns (uint256 r64) {
    ...
    function realNetAssetValueWithoutAssetBalance(Payload payload) internal view returns (uint256 r64) {
        for (uint256 i = 0; i < payload.tokensOwned.length; i++) {
    ...
    function bestNetAssetValue(Payload payload) internal view returns (uint256 r64) {
        r64 += bestNetAssetValueWithoutAssetBalance(payload);
        r64 += payload.asset.balanceR64();
        return r64;
    }

    function realNetAssetValue(Payload payload) internal view returns (uint256 r64) {
    ...
    function hasEnoughLiquidity(LiquidityCheckPayload payload) internal view returns (bool) {
        ...
        uint256 priceR64 = UniswapV2PairLibrary.priceR64(adaptorPayload);
        if (priceR64 == 0) {
            return false;
        }
        ...
        uint256 slippage = UniswapV2PairLibrary.slippage(adaptorPathPayload);
