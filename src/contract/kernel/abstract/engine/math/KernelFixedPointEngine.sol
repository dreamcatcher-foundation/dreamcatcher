...
abstract contract KernelFixedPointEngine {
    error UnsignedIntegerOverflow(
        uint256 x,
        uint256 y,
        uint256 result);

    error UnsignedIntegerUnderflow(
        uint256 x,
        uint256 y,
        uint256 result);

    error UnsignedIntegerMulDivOverflow();
    ...
    function _muldiv(uint256 x, uint256 y, uint256 z) internal pure returns (uint256) {
        unchecked {
            ...
            if (z > prod1) revert UnsignedIntegerMulDivOverflow();
