...
contract TokenSdk is TokenSocket {
    ...
    function _totalSupply() internal view returns (uint256) {
        return _tokenSocket().totalSupply();
    }

    function _balanceOf(address account) internal view returns (uint256) {
        return _tokenSocket().balanceOf(account);
    }

    function _allowance(address owner, address spender) internal view returns (uint256) {
        return _tokenSocket().allowance(owner, spender);
    }
    ...
    function __transfer(address from, address to, uint256 amount) private returns (bool) {
        ...
        _tokenSocket().setBalanceOf(from, _balanceOf(from) - amount);
        _tokenSocket().setBalanceOf(to, _balanceOf(to) + amount);
    ...
    function __approve(address owner, address spender, uint256 amount) private returns (bool) {
        ...
        _tokenSocket().setAllowanceOf(owner, spender, amount);
